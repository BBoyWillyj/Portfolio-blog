...
export default function TipTapEditor({ content, onChange, placeholder = 'Tell your story...' }: TipTapEditorProps) {
    ...
    const ToolbarButton = ({ onClick, isActive, children, title }: any) => (
        <button
            ...
            onClick={onClick}
            title={title}
            className={`p-2 rounded-lg text-sm transition-all duration-150 cursor-pointer ${isActive
        ...
        <div className="w-full border border-zinc-800 rounded-xl bg-zinc-950/20 backdrop-blur-md overflow-hidden focus-within:border-zinc-700 transition-all duration-200">
            ...
            {editor && (
                <BubbleMenu editor={editor}>
                    <div className="flex bg-zinc-900 border border-zinc-800 p-1.5 rounded-lg shadow-xl gap-1 items-center">
                        <button
                            type="button"
                            onClick={() => editor.chain().focus().toggleBold().run()}
                            className={`px-2 py-1 text-xs rounded font-mono ${editor.isActive('bold') ? 'bg-[#C8522A] text-[#F5F0E8]' : 'text-zinc-400 hover:bg-white/10'}`}
                        >
                            B
                        </button>
                        {/* Rest of your button elements go here */}
                    </div>
                </BubbleMenu>
            )}
